...
// Main Component: Manages team generation and drag-and-drop for High Player Pick format
const HighPlayerPickTeams: React.FC<{
  tournament: Tournament;
  onUpdate: (updatedTournament: Tournament) => void;
  isReadOnly?: boolean;
}> = ({ tournament, onUpdate, isReadOnly = false }) => {
  // State Management
  const [aPlayers, setAPlayers] = useState<(Tournament['players'][0] | null)[]>([]); // Higher stat players
  const [bPlayers, setBPlayers] = useState<(Tournament['players'][0] | null)[]>([]); // Placeholder slots for B players
  const [availablePlayers, setAvailablePlayers] = useState<(Tournament['players'][0] | null)[]>([]); // Lower stat players available for picking
  ...
  const buildTeamsFromLists = useCallback(
    (aPlayersList: (Tournament['players'][0] | null)[], bPlayersList: (Tournament['players'][0] | null)[]): { name: string; players: string[] }[] => {
      const regularTeams = aPlayersList.map((aPlayer, index) => {
  ...
  // Sorts players by the specified stat and assigns them to A (high stat), B (placeholders), and Available lists
  const dividePlayers = useCallback(
    async (stat: 'combo' | 'ppd' | 'mpr') => {
      ...
      const newAPlayers = sortedPlayers.slice(0, middleIndex); // Higher stat players
      const newBPlayers = new Array(middleIndex).fill(null); // B players are placeholders
      const newAvailablePlayers = [null, ...sortedPlayers.slice(middleIndex)]; // Lower stat players + placeholder
